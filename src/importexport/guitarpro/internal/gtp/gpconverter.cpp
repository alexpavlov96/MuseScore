    ...
    void GPConverter::addContinuousSlideHammerOn()
    {
        ...
        for (const auto& slide : _slideHammerOnMap) {
            ...
            if (slide.second == SlideHammerOn::LegatoSlide || slide.second == SlideHammerOn::Slide) {
                ...
                int midiProgram = startNote->chord()->part()->instrument()->channel(0)->program();
                LOGE() << "@# glissando! midi = " << midiProgram << ", portamento = " << isPortamento(midiProgram);

                if (isPortamento(midiProgram)) {
                    gl->setGlissandoStyle(mu::engraving::GlissandoStyle::PORTAMENTO);
                }
